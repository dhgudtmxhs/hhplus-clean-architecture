...
도메인 주도 설계(DDD) 관점에서,
	...
	•	이 경우, LectureRegistration이 어디에 속하는가? → “강의” 애그리거트 안에 속한다고 보는 것이 일반적입니다.
 	...
 	•	응집도(E cohesiveness): Lecture와 LectureRegistration은 긴밀하게 연관된 비즈니스 로직을 공유합니다. 예를 들어, 강의 등록 시 정원 관리, 중복 신청 방지 등이 하나의 흐름으로 이어집니다.
	•	단일 책임 원칙(Single Responsibility Principle): 하나의 서비스가 특정 도메인의 비즈니스 로직을 일관되게 처리할 수 있습니다.
	•	유지보수성 및 가독성: 관련된 로직이 하나의 서비스에 모여 있어, 코드를 이해하고 수정하기 용이합니다.

리포지토리는 각 엔티티별로 분리하는 것이 바람직합니다.

이유:
	•	단일 책임 원칙(Single Responsibility Principle): 각 리포지토리가 특정 엔티티에 대한 데이터 접근 로직만을 담당합니다.
	•	유지보수성 및 테스트 용이성: 특정 엔티티에 대한 쿼리나 데이터 접근 로직을 변경할 때, 해당 리포지토리만 수정하면 되므로 유지보수가 용이합니다.
	•	재사용성: 다른 서비스나 도메인에서도 특정 리포지토리를 재사용할 수 있습니다.
